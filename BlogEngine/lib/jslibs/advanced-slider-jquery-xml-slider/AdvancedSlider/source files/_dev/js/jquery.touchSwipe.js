 ...
 * $version: 1.2.5 	- Added support for testing swipes with mouse on desktop browser (thanks to https://github.com/joelhy)
 ...
 Added support for IE6-IE8
 Added the event listeners to window/document in order to catch release outside events
		...
		return this.each(function() 
		{
			...
			*/
			function touchStart(event) 
			{
				...
				if (hasTouch) {
					document.addEventListener(MOVE_EV, touchMove);
					document.addEventListener(END_EV, touchEnd);
				} else {
					$(document).bind(MOVE_EV, touchMove);
					$(document).bind(END_EV, touchEnd);	
				}
			}
			...
			*/
			function touchEnd(event) 
			{
				...
				if (hasTouch) {
					document.removeEventListener(MOVE_EV, touchMove);
					document.removeEventListener(END_EV, touchEnd);
				} else {
					$(document).unbind(MOVE_EV, touchMove);
					$(document).unbind(END_EV, touchEnd);
				}
			}
			...
			*/
			...
			if (hasTouch) {
				that.addEventListener(START_EV, touchStart);
				document.addEventListener(CANCEL_EV, touchCancel);
			} else {
				$this.bind(START_EV, touchStart);
				$(document).bind(CANCEL_EV, touchCancel);
			}
